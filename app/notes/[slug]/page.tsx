...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!note) {
    return {
      title: 'Note Not Found | Hendry Bambang Siswanto',
    };
  }

  return {
    title: `${note.meta.title} | Hendry Bambang Siswanto`,
      ...
      <article className="space-y-8">
        ...
        <header className="space-y-3 pb-6 border-b border-border">
          <div className="flex items-center gap-3 text-xs text-text-secondary font-semibold uppercase tracking-wider">
            <time dateTime={note.meta.date}>{note.meta.date}</time>
            {note.meta.readTime && <span>• {note.meta.readTime}</span>}
          </div>
